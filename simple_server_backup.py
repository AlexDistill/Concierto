                    ...
                    const contentHtml = data.items.map(item => {
                        if (item.type === 'image') {
                            ...
                            return '<div class="content-item" onclick="editItem(\'' + item.id + '\')" style="cursor: pointer;">' +
                                    '<img src="/' + item.path + '" alt="' + item.title + '" class="image-preview" onerror="this.style.display=\'none\'">' +
                                    '<div class="item-content">' +
                                        '<div class="item-title">' + item.title + '</div>' +
                                        description +
                                        (item.notes ? '<p class="item-notes" style="color: #764ba2; font-style: italic; margin: 0.5rem 0;">📝 ' + item.notes + '</p>' : '') +
                                        '<div class="item-tags">' +
            ...
            const contentHtml = items.map(item => {
                if (item.type === 'image') {
                    ...
                    return '<div class="content-item" onclick="editItem(\'' + item.id + '\')" style="cursor: pointer;">' +
                            '<div class="item-content">' +
                                '<div class="item-title">' + item.title + '</div>' +
                                '<p>' + (item.content ? item.content.substring(0, 150) : '') + (item.content && item.content.length > 150 ? '...' : '') + '</p>' +
                                '<div class="item-tags">' +
                                    (item.tags || []).map(tag => '<span class="tag">' + tag + '</span>').join('') +
                                '</div>' +
                            '</div>' +
